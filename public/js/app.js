...
function getMensajes() {
    fetch('api')
        .then(res => res.json())
        .then(post => {
            post.forEach(post => crearMensajeHTML(post.mensaje, post.user));
        })
}
...
function getPublicKey() {
    ...
    return fetch('api/key')
        ...
        // Regresar el arreglo como un Uint8Array
...
btnDesactivadas.on('click', () => {
    if (!swReg) return console.log('No hay registro de service worker');
